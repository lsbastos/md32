...
SHHS <- read_csv("dados/shhs.csv")
racao <- read_csv("dados/racao.csv")
dmft <- read_csv("dados/dmft.csv")

# Definindo categorias de base
racao$Linhagem = racao$Linhagem |> as.factor() |> relevel(ref = "Swiss")
racao$Racao = racao$Racao |> as.factor() # |> relevel(ref = "Controle")
...
ks.test(rstandard(modelo), y="pnorm")
...
## ----echo=F, fig=T---------------------------------------------------------------
SHHS |> 
  ggplot(aes(x = Smoking, y = BMI, fill = Smoking)) + 
  geom_hline(yintercept = c(25, 30), linetype = "dashed") + 
  geom_boxplot( show.legend = F, alpha = 0.75 ) + 
  theme_light( base_size = 14)
...
## ----echo=F, fig=T---------------------------------------------------------------
SHHS |> 
  ggplot(aes(x = Smoking, y = BMI, fill = Sex)) + 
  geom_hline(yintercept = c(25, 30), linetype = "dashed") + 
  geom_boxplot( show.legend = F, alpha = 0.75 ) + 
  theme_light( base_size = 14)
