...
library(xtable)
set.seed(42) # Fixando a semente para garantir replicabilidade
...
p = lm.ex$rank # Numero de parametros
...
## ----echo=FALSE, fig=TRUE--------------------------------------------------------
ggplot(dados, aes(x=x2, y=y, fill = factor(x2))) + 
...
## ----echo=FALSE, fig=TRUE--------------------------------------------------------
bbb <- shapiro.test(dados2$res[-c(200,300)])
qqnorm(dados2$res[-c(200,300)])
qqline(dados2$res[-c(200,300)])
text(x = 1, y = min(dados2$res), label = paste(bbb$method, ": p-value=", round(bbb$p.value,3)))
...
## ----echo=FALSE, fig=TRUE--------------------------------------------------------
hist(dados2$res[-c(200,300)], col = "red")
